...
import { getOrderAdminApi, trackOrderApi } from "../../../../services/adminApiRoutes";
...
const AdminOrderDetail = () => {
  ...
  const getTrackOrder = async () => {
    try {
      const response = await trackOrderApi(orderData?.id);
  ...
  const defaultSteps = [
    { id: 1, name: "confirmed", title: "Confirmed", date: "", time: "" },
    { id: 2, name: "accepted", title: "Accepted", date: "", time: "" },
    { id: 3, name: "dispatched", title: "Ready to dispatch", date: "", time: "" },
    { id: 4, name: "out_of_delivery", title: "Out of delivery", date: "", time: "" },
    { id: 5, name: "delivered", title: "Delivered", date: "", time: "" },
  ];

  // Filter out the 'confirmed' step
  const stepsline = defaultSteps.filter((items) => items.name !== "confirmed");

  // Map trackOrder data to stepsline
  const updatedSteps = stepsline.map((step) => {
    const trackOrderStep = trackOrder.find((order) => order.status === step.name);
    return {
      ...step,
  ...
  const currentStepIndex = updatedSteps.findIndex((step) => step.name === currentStatus);
                    ...
                    <div>
                      <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                        {updatedSteps.map((step, index) => (
                          <div
                            className={`d-flex gap-3 stepper-wrapper-order ${index <= currentStepIndex ? "active" : ""}`}
                            key={step.id}
                          >
                            <div
                              style={{
                            ...
                            <div className="d-flex flex-column justify-content-start align-items-start stepper-content">
                              <h6 className="fw-400 mb-0" style={{ fontSize: "1rem" }}>
                                {step.title}
                              </h6>
                              {index <= currentStepIndex && (
                                <p className="fw-400" style={{ fontSize: "0.875rem", color: "#918E92" }}>
                                  {step.date || ""} {step.time || ""}
                                </p>
                              )}
                            </div>
